...
Usage:
    ...
    python render_trajectory.py --data-dir /path/to/data --ckpt /path/to/ckpt.pt --output video.mp4 --up-offset 0.1

    # Render ellipse trajectory
    python render_trajectory.py --data-dir /path/to/data --ckpt /path/to/ckpt.pt --output video.mp4 --traj-type ellipse

    # Render using original COLMAP camera poses
    python render_trajectory.py --data-dir /path/to/data --ckpt /path/to/ckpt.pt --output video.mp4 --traj-type colmap

    # Render using COLMAP poses with interpolation (smoother video)
    python render_trajectory.py --data-dir /path/to/data --ckpt /path/to/ckpt.pt --output video.mp4 --traj-type colmap --colmap-interp 3
"""
...
def render_trajectory_comparison(
    ...
    cv2.putText(
        image, "Original", (20, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 100, 255), 2
    )
    cv2.putText(
        image, "Offset", (20, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255, 50, 50), 2
    )
...
def render_trajectory_comparison_video(
    ...
    for i in tqdm.trange(
        len(viewpoint_trajectory), desc="Rendering trajectory comparison"
    ):
        ...
        cv2.putText(
            image, "Original", (20, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 100, 255), 2
        )
        cv2.putText(
            image, "Offset", (20, 55), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 50, 50), 2
        )
...
def render_dual_trajectory_video(
    ...
    for i in tqdm.trange(n_frames, desc="Rendering dual trajectory"):
        ...
        depths_orig = (depths_orig - depths_orig.min()) / (
            depths_orig.max() - depths_orig.min() + 1e-8
        )
        ...
        depths_offset = (depths_offset - depths_offset.min()) / (
            depths_offset.max() - depths_offset.min() + 1e-8
        )
...
Examples:
  ...
  python generate_traj.py --data-dir ./data -o traj.npz --save-traj-only
    ...
    parser.add_argument(
        "--data-dir", type=str, required=True, help="Path to COLMAP dataset directory"
    )
    parser.add_argument(
        "--ckpt",
        type=str,
    ...
    parser.add_argument(
        "--offset-x", type=float, default=0.0, help="X offset in meters (world space)"
    )
    parser.add_argument(
        "--offset-y", type=float, default=0.0, help="Y offset in meters (world space)"
    )
    parser.add_argument(
        "--offset-z",
        type=float,
    ...
    parser.add_argument("--pitch", type=float, default=0.0, help="Pitch of viewing")
    parser.add_argument(
        "--elevation", type=float, default=None, help="elevateion of viewing"
    )
    ...
    try:
        ...
        print(
            "  Continuing anyway - images may not be needed for trajectory generation"
        )
    ...
    # Get default camera intrinsics and image size from parser
    ...
    if args.traj_file is not None:
        ...
        if not traj_path.exists():
            ...
            print("  Try using absolute path or check file location.")
        ...
        if traj_suffix == ".json":
            ...
            original_camtoworlds = traj_data.camtoworlds
            ...
            raise ValueError(f"Unsupported suffix {traj_suffix}")
    ...
    if args.max_frames is not None and args.max_frames > 0:
        if len(original_camtoworlds) > args.max_frames:
            print(
                f"Limiting trajectory from {len(original_camtoworlds)} to {args.max_frames} frames"
            )
    ...
    if args.save_traj_only or not is_video:
        ...
        save_trajectory(
            offset_camtoworlds, output_path, intrinsics=K, image_size=(width, height)
        )
    else:
        ...
        print(
            f"Loaded checkpoint from step {step} with {len(splats['means'])} Gaussians"
        )
        ...
        if has_offset and not args.no_comparison:
            ...
            comparison_image_path = str(
                output_path.with_stem(output_path.stem + "_comparison").with_suffix(
                    ".png"
                )
            )
            comparison_video_path = str(
                output_path.with_stem(output_path.stem + "_comparison")
            )
            ...
            comparison_frames = original_camtoworlds[
                :: max(1, len(original_camtoworlds) // 120)
            ]
